    ...
    def display(self):
        ...
        for line in self.tracked_hours:
            ...
            if line in self.start_dates and is_start_date:
                ...
                print(f'Start date: {self.format_date(temp_date,1)}',format('Hours','>20s'))
                #mm/dd:hours + int(hours) + (/'text')
                if breakthrough:
                    print(format(temp_date_m+':','<6s')+format('*'*int(temp_hours),'<16s'),format(temp_hours,f'>18s'),'  /'+self.text[temp_date_y])
                else:
                    print(format(temp_date_m+':','<6s')+format('*'*int(temp_hours),'<16s'),format(temp_hours,f'>18s'))
                streak+=1
                if int(temp_hours)==0:
                    streak = 0
                is_start_date = False
            ...
            #show continued date
                ...
                #mm/dd:hours + int(hours) + (/'text')
                if breakthrough:
                    print(temp_date_m+':'+format('*'*int(temp_hours),'<16s'),format(temp_hours,f'>18s'),'  /'+self.text[temp_date_y])
                else:
                    print(temp_date_m+':'+format('*'*int(temp_hours),'<16s'),format(temp_hours,f'>18s'))
                streak+=1
                if int(temp_hours)==0:
                    streak = 0
            ...
            #show rest of dates
                ...
                #breakthrough day
                    ...
                    #mm/dd:hours + int(hours) + /'text'
                    print(temp_date_m+':'+format('*'*int(temp_hours),'<16s'),format(temp[1],f'>18s'),'  /'+self.text[temp_date_y])
                    streak+=1
...
if len(memory)==0:
    ...
    #create and store the goal in memory
    goal = Goal(new_goal)
    goal.create()
...
while running:
    ...
    if action=='':
        ...
        if action=='':
            ...
            pass
    while True:
        action = input('"l" to list all goals, "c" to change goal, "n" to create new goal, "q" to quit: ')
        if action=='l':
            print()
            print('Current goals:')
            ...
            try:
                ...
                print('File data for goal not found')
                print()
                goal = temp
            else:
                goal.store_in_memory()
